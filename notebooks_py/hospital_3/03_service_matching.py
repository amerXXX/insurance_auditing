# Hospital 3 pipeline, part 3/7 — depends on 01_setup.py and 02_contract_extraction.py
# (uses line_items, rate_schedule, threshold_premiums, weekend_uplifts, volume_discounts,
# bundles, substituted_rates).

### Service matching
#
# Hospital 3's billing descriptions use the same abbreviation conventions and free-text
...
#
# plausible_unit_prices() is used only as secondary evidence to break a text tie between
# structurally-compatible candidates. Hospital 3 needs one adaptation the other hospitals
# did not: a Service with a substituted rate legitimately carries two different prices
# during the term -- Appendix B's rate for Service Dates before 1 January 2025 and the
# amendment's rate from that date. The observed prices for one description therefore span
# both periods, so the plausible set is built across both, plus the usual premium, uplift,
# discount and bundle variants. Clause 1.5 removes both multipliers.
...
_substituted_lookup = dict(zip(substituted_rates["service"], substituted_rates["rate_from_cents"]))
...
def plausible_unit_prices(service):
    ...
    Spans both rate periods, since a substituted-rate Service is billed at Appendix B's
    rate before 1 January 2025 and at the amendment's rate from that date.
    """
    if service in _plausible_cache:
        return _plausible_cache[service]

    starting_rates = {int(_base_rate_lookup[service])}
    if service in _substituted_lookup:
        starting_rates.add(int(_substituted_lookup[service]))
...
# ---- Resolve every distinct billing description against the Hospital 3 rate schedule ----
