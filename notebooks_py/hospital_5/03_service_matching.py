# Hospital 5 pipeline, part 3/7 — depends on 01_setup.py and 02_contract_extraction.py
# (uses line_items, rate_schedule, threshold_premiums, weekend_uplifts, volume_discounts,
# bundles, FACILITY_MULTIPLIER, TIER_MULTIPLIER).

### Service matching
#
# Hospital 5's billing descriptions use the same abbreviation conventions, the same
# adjective/specialty/service-class vocabulary, and the same free-text style as Hospitals
# 1 and 4 -- this is the same synthetic generator across hospitals. The matcher, its alias
# tables, and its conservative acceptance thresholds (MIN_SCORE=0.55, GAP_THRESHOLD=0.08)
# are therefore reused verbatim rather than re-tuned, keeping the audit conservative in
# exactly the same way: unmatched or ambiguous descriptions go to needs_review rather than
# being guessed or silently treated as correct.
#
# One necessary adaptation: plausible_unit_prices() models the prices a line of a given
# service could legitimately carry, and is used only as secondary evidence to break a
# text tie. For Hospitals 1 and 4 that was base/premium/discount-derived. Hospital 5's
# billed prices also carry a facility multiplier and a plan-tier multiplier, so a set
# built from unmultiplied rates would match nothing and the tie-break would be inert.
# The set is therefore built across every facility x tier combination for the service
# (and, unlike Hospitals 1 and 4, across its bundled substituted rate too -- closing the
# gap their decision logs recorded as a known limitation). A wider plausible set makes it
# *less* likely that exactly one candidate qualifies, so the failure direction is more
...
def plausible_unit_prices(service):
    ...
    Mirrors the pricing engine: base (or bundled substitute) -> facility multiplier ->
    plan-tier multiplier, with half-up rounding after each step, then optional
    premium / weekend uplift / volume discount variants.
    """
    ...
    premium_pcts = [int(p) for p in threshold_premiums.loc[
        threshold_premiums["service"] == service, "premium_percent"]]
    weekend_pcts = [int(p) for p in weekend_uplifts.loc[
        weekend_uplifts["service"] == service, "weekend_uplift_percent"]]
    discount_pcts = [int(p) for p in volume_discounts.loc[
        volume_discounts["service"] == service, "discount_percent"]]

    prices = set()
    for start in starting_rates:
        for facility in FACILITY_CODES:
            after_facility = half_up(Decimal(start) * FACILITY_MULTIPLIER[(service, facility)])
            for tier in PLAN_TIERS:
                after_tier = half_up(Decimal(after_facility) * TIER_MULTIPLIER[(service, tier)])
                uplifted = [after_tier]
                uplifted += [half_up(Decimal(after_tier) * (Decimal(100 + p) / 100)) for p in premium_pcts]
                uplifted += [half_up(Decimal(after_tier) * (Decimal(100 + p) / 100)) for p in weekend_pcts]
...
# ---- Resolve every distinct billing description against the Hospital 5 rate schedule ----
